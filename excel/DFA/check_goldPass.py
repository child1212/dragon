        ...
        if i > lenth-28:
            if i == lenth-27:
                ...
                if gpl[i,11] == gpl[i-27,11]:
                    if str(gpl[i,11]) != "nan":
                        print(gpl[i,0],":",gpl[0,11],"--",gpl[i,11],"奖励一样了")
...
for i in range(3,lenth_cfg):
    if cfg[i,0] == "goldPassPerk":
        ...
        for j in json.loads(cfg[i,2]):
            vv.add(tuple(j))
        ww = {(gpid,1,2,5,10),(gpid-1,1,2,5,10),(gpid-2,1,2,5,10)}
        if vv != ww:
    ...
    if cfg[i,0] == "goldPassFileIndex":
        ...
        for j in json.loads(cfg[i,2]):
            vv.add(tuple(j))
        if gpid%3 == 0:
            ww = {(gpid,3),(gpid-1,2),(gpid-2,1)}
        elif gpid%3 == 1:
            ww = {(gpid,1),(gpid-1,3),(gpid-2,2)}
        elif gpid%3 == 2:
            ww = {(gpid,2),(gpid-1,1),(gpid-2,3)}
        if vv != ww:
            print("config:黄金通行证特权配置错误",cfg[i,0])
...
for i in range(3,lenth_cfg):
    if cfg[i,0] == "goldPassPerk":
        ...
        for j in json.loads(cfg[i,2]):
            vv.add(tuple(j))
        ww = {(gpid,1,2,5,10),(gpid-1,1,2,5,10),(gpid+1,1,2,5,10)}
        if vv != ww:
            print("config:黄金通行证特权配置错误",cfg[i,0])
    ...
    if cfg[i,0] == "goldPassIntegralBuffValue":
        ...
        for j in json.loads(cfg[i,2]):
            vv.add(tuple(j))
        ww = {(gpid,1200),(gpid-1,1200),(gpid+1,1200)}
        if vv != ww:
            print("config:黄金通行证特权配置错误",cfg[i,0])
    ...
    if cfg[i,0] == "goldPassCovertGoldValue":
        ...
        for j in json.loads(cfg[i,2]):
            vv.add(tuple(j))
        ww = {(gpid,1000),(gpid-1,1000),(gpid+1,1000)}
        if vv != ww:
            print("config:黄金通行证特权配置错误",cfg[i,0])
    ...
    if cfg[i,0] == "goldPassFileIndex":
        ...
        for j in json.loads(cfg[i,2]):
            vv.add(tuple(j))
        if gpid%3 == 0:
            ww = {(gpid,3),(gpid-1,2),(gpid+1,1)}
        elif gpid%3 == 1:
            ww = {(gpid,1),(gpid-1,3),(gpid+1,2)}
        elif gpid%3 == 2:
            ww = {(gpid,2),(gpid-1,1),(gpid+1,3)}
        if vv != ww:
            print("config:黄金通行证特权配置错误",cfg[i,0])
