...
for country in countries:
    for name in names:
        lan = open("E:\\town\\FA\\client\\client\\Assets\\HomeLand\\Localization\\{country}\\{name}.bytes".format(country=country,name=name),"r",encoding="utf-8")
        val = open("D:\\pyprogram\\PyTestTools\\dragon\\NFA\\local\\{country}_{name}.txt".format(country=country,name=name),"w",encoding="utf-8")
        for line in lan:
            if "{" in line:
                keys = line.split("=")
                key = keys[0]
...
#执行比对
val = open("D:\\pyprogram\\PyTestTools\\dragon\\NFA\\local\\zh_UI.txt".format(name=name),"r",encoding="utf-8")
stand = {}
for line in val:
    kx = line.split(':')
    kk = kx[1].split(",")
...
for country in countries:
    print("#===================="+country+"=======================#")
    comp = {}
    lan = open("D:\\pyprogram\\PyTestTools\\dragon\\NFA\\local\\{country}_UI.txt".format(country=country),"r",encoding="utf-8")
    for line in lan:
        kx = line.split(':')
        kk = kx[1].split(",")
        kr = set(kk)
...
for name in names:
    ...
    for country in countries:
        ...
        val.close()
    #创建对照库
    val = open("D:\\pyprogram\\PyTestTools\\dragon\\NFA\\local\\zh_{name}.txt".format(name=name),"r",encoding="utf-8")
    stand = set()
    for line in val:
        line = line.replace("\n",'')
        if line in stand:
    ...
    for country in countries_compare:
        print("#===================="+country+"=======================#")
        comp = set()
        lan = open("D:\\pyprogram\\PyTestTools\\dragon\\NFA\\local\\{country}_{name}.txt".format(country=country,name=name),"r",encoding="utf-8")
        for line in lan:
            line = line.replace("\n",'')
            comp.add(line)
        for k in stand:
